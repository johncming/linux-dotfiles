...
augroup filetypedetect
  ...
  autocmd BufRead,BufNewFile *.rule set filetype=yaml
...
" fold
nnoremap F :set foldmethod=indent<CR>zM
...
let g:go_decls_includes = 'type,func'
...
nmap <C-g> :GoDecls<cr>
nmap <C-l> :GoDeclsDir<cr>

" yank filename and current line number for debug in dlv
nnoremap ,m :let @*=expand('%:p').":".line(".")<CR>
...
" 翻译注释
function! s:translateComment()
  exe "normal yac"
  let buf=substitute(@", '// ', '', 'g')
  let @+=substitute(buf, '\n', ' ', 'g')
  call system('paste_to_deepl')
endfunction

" 去除 vim-go 中 test.go中的refer
function! s:removeTest()
  let l:content = getqflist()
  let l:newContent = []
  for line in l:content
    let l:fname = bufname(line.bufnr)
    if strridx(l:fname, "_test.go") == -1
      let l:idx = line.lnum - 1
      call add(l:newContent, line)
    endif
  endfor
  call setqflist(l:newContent, 'r')
endfunction


" 生成断点并落盘
function! s:saveqf(clearall)
  let l:content = getqflist()
  let l:newContent = []
  let l:writeMode = "a"
  call add(l:newContent, "############### ")
  if a:clearall 
    call add(l:newContent, "clearall")
    let l:writeMode = ""
  endif
  for line in l:content
    let l:fname = fnamemodify(bufname(line.bufnr), ":p")
    if stridx(l:fname, "_test.go") == -1
      let l:newLine = l:fname.":".line.lnum
      call add(l:newContent, "b ".l:newLine)
    endif
  endfor
  call writefile(l:newContent, "bp.cmd", l:writeMode)
endfunction


function! s:whichFunc()
  let save_pos = getpos(".")
  execute "normal! 9[{"
  execute "normal! yy"
  ...
  let l:content = strpart(l:content, 0, l:idx-1)
  call popup_notification(l:content, {
        \ "line": "cursor-1",
        \ "col" : "cursor",
        \ "pos": "botright",
        \ "time": "30000",
